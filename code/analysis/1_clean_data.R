...
safe_bind_rows <- function(df_social, df_dynata) {
    ...
    for (col in common_cols) {
        if (typeof(df_social[[col]]) != typeof(df_dynata[[col]])) {
            ...
            if (
                is.character(df_social[[col]]) || is.character(df_dynata[[col]])
            ) {
                df_social[[col]] <- as.character(df_social[[col]])
                df_dynata[[col]] <- as.character(df_dynata[[col]])
            } else if (
                is.numeric(df_social[[col]]) && is.numeric(df_dynata[[col]])
            ) {
    ...
    if (nrow(df_social) == 0 && nrow(df_dynata) == 0) {
        ...
        return(reorder_and_add_columns(
            df_dynata,
            names(df_social),
            character(0)
        ))
...
carData2 <- carData2 %>%
    ...
    mutate(
        ...
        time_sec_2 = as.numeric(time_2_ended - time_2_created, units = "secs")
    ) %>%
    # Select important columns
    select(
        session,
...
carData4b <- carData4b %>%
    ...
    mutate(
        ...
        time_sec_4b = as.numeric(
            time_4b_ended - time_4b_created,
            units = "secs"
        )
    ) %>%
    # Select important columns
...
save(
    choice_data_wider,
    file = file.path(processed_dir, "choice_data_wider.RData")
)

write_csv(
    choice_data_wider_smc,
    file.path(processed_dir, "choice_data_wider_smc.csv")
)
save(
    choice_data_wider_smc,
    file = file.path(processed_dir, "choice_data_wider_smc.RData")
)

write_csv(
    choice_data_wider_v2g,
    file.path(processed_dir, "choice_data_wider_v2g.csv")
)
save(
    choice_data_wider_v2g,
    file = file.path(processed_dir, "choice_data_wider_v2g.RData")
)
...
smc_survey <- read_csv(
    "https://raw.githubusercontent.com/pingfan-hu/My-Resources/main/bev/smc_questions.csv"
) %>%
...
smc_data$respID <- rep(
    seq(smc_n_respondents),
    each = smc_n_alts * smc_n_questions
)
smc_data$obsID <- rep(
    seq(smc_n_respondents * smc_n_questions),
    each = smc_n_alts
)
...
v2g_survey <- read_csv(
    "https://raw.githubusercontent.com/pingfan-hu/My-Resources/main/bev/v2g_questions.csv"
)
...
v2g_data <- v2g_data %>%
    ...
    select(
        ...
        enrollment_cash,
        occurrence_cash,
        starts_with("monthly_occurrence"),
        lower_bound,
        guaranteed_threshold,
        choice,
        no_choice,
...
v2g_data$respID <- rep(
    seq(v2g_n_respondents),
    each = v2g_n_alts * v2g_n_questions
)
v2g_data$obsID <- rep(
    seq(v2g_n_respondents * v2g_n_questions),
    each = v2g_n_alts
)
