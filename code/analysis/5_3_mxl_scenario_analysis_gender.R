...
load(file.path(processed_dir, "2_models", "smc_mxl_model_gender.RData"))
...
smc_scenarios <- smc_scenarios %>%
    mutate(
        no_choice = ifelse(alt_id == 2, no_choice + asc_adjustment, no_choice)
    )

smc_mxl_scenario <- predict(
    smc_mxl_model,
...
smc_scenarios_grouped <- data.frame(
    scenario_id = 1:6,
    attribute = rep(
        c("One-time Cash", "Recurring Cash", "Flexibility"),
        each = 2
    ),
    level = c(
        "$50 Enrollment Cash",
        ...
        "$2 Monthly Cash",
        "$20 Monthly Cash",
        "1 Day Override + 20/40% Battery",
        "5 Days Override + 40/80% Battery"
    )
)

smc_mxl_scenario_plot <- smc_mxl_scenario %>%
    filter(alt_id == 1) %>%
    ...
    mutate(
        attribute = factor(
            attribute,
            levels = c("One-time Cash", "Recurring Cash", "Flexibility")
        )
    ) %>%
    ggplot(aes(
        x = predicted_prob,
    ...
    geom_errorbar(
        aes(xmin = predicted_prob_lower, xmax = predicted_prob_upper),
        position = position_dodge(width = 0.8),
        width = 0.4
    ) +
    geom_text(
        aes(
            x = predicted_prob_upper + 0.01,
    ...
    geom_text(
        aes(x = 0.02, label = level, y = attribute),
        position = position_dodge(width = 0.8),
        hjust = 0,
        vjust = 0.5,
        size = 3.5,
...
smc_mxl_scenario_plot

ggsave(
    filename = file.path(
        processed_dir,
        "4_scenario_analysis",
        "smc_mxl_scenario_plot_gender.png"
    ),
    plot = smc_mxl_scenario_plot,
    width = 7,
    height = 7 / 1.618
)
...
load(file.path(processed_dir, "2_models", "v2g_mxl_model_gender.RData"))
...
v2g_scenarios_grouped <- data.frame(
    ...
    level = c(
        "$50 Enrollment Cash",
        "$300 Enrollment Cash",
        "$2 Occurrence Cash, 1 per Month",
        "$20 Occurrence Cash, 4 per Month",
        "20/40% Battery",
        "40/80% Battery"
    )
)

v2g_mxl_scenario_plot <- v2g_mxl_scenario %>%
    filter(alt_id == 1) %>%
...
v2g_mxl_scenario_plot

ggsave(
    filename = file.path(
        processed_dir,
        "4_scenario_analysis",
        "v2g_mxl_scenario_plot_gender.png"
    ),
    plot = v2g_mxl_scenario_plot,
    width = 7,
    height = 7 / 1.618
)
...
mxl_scenario_plots <-
    (wrap_elements(full = smc_mxl_scenario_plot)) /
    (wrap_elements(full = v2g_mxl_scenario_plot))
mxl_scenario_plots
...
ggsave(
    filename = file.path(
        processed_dir,
        "4_scenario_analysis",
        "mxl_scenario_plots_gender.png"
    ),
    plot = mxl_scenario_plots,
    width = 10,
    height = 10 / 1.618
)
