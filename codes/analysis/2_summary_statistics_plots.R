...
choice_data_ownership <- choice_data_wider %>%
    mutate(across(c(mc_car_number, has_bev, mc_commute_method, mc_commute_days,
                    mc_distance, mc_neighbor), as.character)) %>%
    pivot_longer(
        cols = c(mc_car_number, has_bev, mc_commute_method, mc_commute_days,
                 mc_distance, mc_neighbor),
        names_to = "category",
        values_to = "value"
    )
    ...
    mutate(
        ...
        value = case_when(
            category == "Household Car Number" ~
                factor(value, levels = c("0", "1", "2", "3", "4", "5"), 
                       labels = c("0", "1", "2", "3", "4", "5 or More")),
            category == "Own BEV" ~
                factor(value, levels = c("yes", "no"),
                labels = c("Yes", "No but Plan to")),
            category == "Commute Method" ~
                factor(value, levels = c("by_car", "by_bus", "by_train", "by_walk", "NA"),
                       labels = c("Car", "Bus", "Train", "Walk", "No Need")),
            category == "Commute Days" ~
                factor(value, levels = c("NA", "1", "2", "3", "4", "5", "6", "7"),
                       labels = c("0", "1", "2", "3", "4", "5", "6", "7")),
            category == "Daily Distance" ~
                factor(value, levels = c("below_10", "10_to_30", "31_to_50",
                                         "51_to_100", "over_100", "no"),
                       labels = c("<10", "10-30", "31-50",
                                  "51-100", ">100", "Don't Drive")),
            category == "Neighbor Ownership" ~
                factor(value, levels = c("yes", "no", "not_sure"),
...
ownership_plot <- choice_data_ownership %>%
    ...
    geom_text(stat = "count", aes(label = after_stat(count)),
              vjust = -0.3, size = 3, family = "Ubuntu") +
    scale_y_continuous(expand = expansion(mult = c(0, 0.15))) +
    facet_wrap(~fct_relevel(category, "Household Car Number", "Own BEV", "Commute Method",
                            "Commute Days", "Daily Distance", "Neighbor Ownership"),
...
choice_data_charging <- choice_data_wider %>%
    mutate(across(c(mc_home_charge, mc_home_days, mc_home_hours,
                    mc_work_charge, mc_work_days, mc_work_hours,
                    mc_umc, mc_lv2_charger), as.character)) %>%
    pivot_longer(
        cols = c(mc_home_charge, mc_home_days, mc_home_hours,
                 mc_work_charge, mc_work_days, mc_work_hours,
                 mc_umc, mc_lv2_charger),
        names_to = "category",
        values_to = "value"
    )
        ...
        value = case_when(
            category == "Home Charge" ~
                factor(value, levels = c("yes", "no"), 
                       labels = c("Yes", "No")),
            category == "Home Days" ~
                factor(value, levels = c("1", "2", "3", "4", "5", "6", "7", "NA"),
                       labels = c("1", "2", "3", "4", "5", "6", "7", as.character(NA))),
            category == "Home Hours" ~
                factor(value, levels = c("less_than_3", "3_to_5", "5_to_8", "more_than_8", "NA"),
            ...
            category == "Charge Management" ~
                factor(value, levels = c("app", "smc", "no"),
...
charging_plot <- choice_data_charging %>%
    ...
    geom_text(stat = "count", aes(label = after_stat(count)),
              vjust = -0.3, size = 3, family = "Ubuntu") +
    scale_y_continuous(expand = expansion(mult = c(0, 0.15))) +
    facet_wrap(~fct_relevel(category, "Home Charge", "Home Days", "Home Hours",
                            "Work Charge", "Work Days", "Work Hours",
                            "Charge Management", "Lv2 Charger"),
               scales = "free_x", nrow = 3) +
    labs(x = NULL,
         y = "Count",
...
choice_data_personal_a <- choice_data_wider %>%
    mutate(across(c(mc_climate, mc_gender, mc_work, mc_party,
                    mc_v2g, mc_v2g_charger), as.character)) %>%
    pivot_longer(
        cols = c(mc_climate, mc_gender, mc_work, mc_party, mc_v2g, mc_v2g_charger),
        names_to = "category",
        values_to = "value"
    )
    ...
    mutate(
        ...
        value = case_when(
            category == "Climate Awareness" ~
                factor(value, levels = c("not", "somewhat", "neutral", "believe", "very"), 
                       labels = c("Not", "Somewhat", "Neutral", "Believe", "Very")),
            category == "Gender" ~
                factor(value, levels = c("male", "female", "non_bi", "prefer_not_say"),
                       labels = c("Male", "Female", "Non-Binary", as.character(NA))),
            category == "Work Status" ~
                factor(value, levels = c("student", "employed_under40",
                                         "employed_over40",
                                         "not_employed_yes_looking",
                                         "not_employed_not_looking",
                                         "retired", "disabled", "prefer_not_say"),
                       labels = c("Student", "Part-time", "Full-time",
                                  "Looking", as.character(NA),
                                  "Retired", "Disabled", as.character(NA))),
            category == "Party" ~
                factor(value, levels = c("democratic", "republican", "independent", "prefer_not_say"),
                       labels = c("Democratic", "Republican", "Independent", as.character(NA))),
            category == "V2G Interest" ~
                factor(value, levels = c("yes", "no"),
                       labels = c("Yes", "No")),
            category == "Pay for V2G Charger" ~
                factor(value, levels = c("yes", "no", "already_have", "NA"),
                       labels = c("Willing to", "Don't Want", "Already Have", as.character(NA))),
            TRUE ~ as.factor(value)
        )
    )
...
personal_plot_a <- choice_data_personal_a %>%
    ...
    geom_text(stat = "count", aes(label = after_stat(count)),
              vjust = -0.3, size = 3, family = "Ubuntu") +
    scale_y_continuous(expand = expansion(mult = c(0, 0.15))) +
    facet_wrap(~fct_relevel(category, "Climate Awareness", "Gender", "Work Status",
                            "Party", "V2G Interest", "Pay for V2G Charger"),
               scales = "free_x", nrow = 3) +
    labs(x = NULL,
         y = "Count",
...
choice_data_personal_b <- choice_data_personal_b %>% 
    mutate(
        ...
        value = case_when(
            category == "Ethnicity" ~
                factor(value, levels = c("asian", "black", "white", "hispanic",
                                         "native", "pacific", "other", 
                                         "prefer_not_say"),
                       labels = c("Asian", "Black", "White", "Hispanic",
                                  "Native American", "Pacific Islander",
                                  "Other", "Other")),
            category == "Education" ~
                factor(value, levels = c("no_hs", "hs", "college_some",
                                         "vocational", "degree_associate",
...
personal_plot_b <- choice_data_personal_b %>%
    ...
    geom_text(stat = "count", aes(label = after_stat(count)),
              vjust = -0.3, size = 3, family = "Ubuntu") +
    scale_y_continuous(expand = expansion(mult = c(0, 0.15))) +
    facet_wrap(~fct_relevel(category, "Ethnicity", "Education"),
...
choice_data_personal_c <- choice_data_wider %>%
    ...
    mutate(age_group = factor(age_group, levels = c("<=30", "31-40", "41-50", "51-60", "61-70", ">70", "NA")))
...
choice_data_house_a <- choice_data_wider %>%
    mutate(across(c(mc_house_type, mc_house_owner, mc_household_size), as.character)) %>%
    pivot_longer(
...
choice_data_house_a <- choice_data_house_a %>% 
    mutate(
        ...
        value = case_when(
            category == "House Type" ~
                factor(value, levels = c("mobile", "apartment", "townhome",
                                         "condo", "detached", "other",
                                         "prefer_not_say"),
                       labels = c("Mobile", "Apt", "Townhome", "Condo",
                                  "Detached", "Other", "Other")),
            category == "House Ownership" ~
                factor(value, levels = c("own", "rent", "prefer_not_say"), 
                       labels = c("Own", "Rent", "NA")),
            category == "Household Size" ~
                factor(value, levels = c("1", "2", "3", "4", "5_or_more",
                                         "prefer_not_say"), 
...
house_plot_a <- choice_data_house_a %>%
    ...
    geom_text(stat = "count", aes(label = after_stat(count)),
              vjust = -0.3, size = 3, family = "Ubuntu") +
    scale_y_continuous(expand = expansion(mult = c(0, 0.15))) +
    facet_wrap(~fct_relevel(category, "House Type", "House Ownership", "Household Size"),
               scales = "free_x", nrow = 2) +
    labs(x = NULL,
         y = "Count",
...
house_plot_b <- choice_data_wider %>%
    mutate(mc_income = factor(
        mc_income, levels = c("prefer_not_say", "below_25", "25_to_35", "35_to_50",
                              "50_to_75", "75_to_100", "100_to_150", "150_to_200",
                              "200_to_250", "250_to_300", "300_to_400", "over_400"), 
        labels = c(as.character(NA), "<25k", "25-35k", "35-50k", "50-75k",
                   "75-100k", "100-150k", "150-200k", "200-250k",
                   "250-300k", "300-400k", ">400k"))
    ) %>%
    ggplot(aes(x = mc_income)) + 
    geom_bar(width = 0.6, stat = "count") +
    geom_text(stat = "count", aes(label = after_stat(count)),
              hjust = -0.5, size = 3, family = "Ubuntu") +
    scale_y_continuous(expand = expansion(mult = c(0, 0.15))) +
...
top_bev_plot <- top_bev %>%
    ...
    coord_flip() +
    scale_y_continuous(expand = expansion(mult = c(0, 0.15))) +
    labs(title = "Top 10 BEV Models",
         subtitle = "Demographics Summary 8",
         x = "Car Make & Model", y = "Count") +
    theme_bw(base_family = "Ubuntu") +
...
tesla_ownership <- choice_data_wider %>%
    mutate(owns_tesla = if_else(grepl("^tesla", user_car_1, ignore.case = TRUE) | 
                                    grepl("^tesla", user_car_2, ignore.case = TRUE), 
                                "yes", "no"))

tesla_owner_plot <- tesla_ownership %>%
    mutate(owns_tesla = factor(owns_tesla,
                               levels = c("yes", "no"), 
                               labels = c("Yes", "No"))) %>%
...
tesla_party <- choice_data_wider %>%
    ...
    mutate(owns_tesla = factor(owns_tesla,
                               levels = c("yes", "no"), 
                               labels = c("Yes", "No")),
           mc_party = factor(mc_party,
                             levels = c("democratic", "republican",
                                        "independent", "prefer_not_say"),
                             labels = c("Democratic", "Republican",
                                        "Independent", "Prefer not to say"))) %>%
...
tesla_party_plot <- tesla_party %>%
    ...
    theme_bw(base_family = "Ubuntu") +
    facet_wrap(~fct_relevel(category, "Tesla Owner Party", "Non-Tesla Owner Party"),
               scales = "free_x", nrow = 2)
