...
smc_mnl_model <- logitr(
    ...
    outcome = "choice",
    obsID   = "obs_id",
    pars    = c("enrollment_cash", "monthly_cash", "override_days",
                "override_flag", "minimum_threshold", "guaranteed_threshold",
                "no_choice")
)
...
smc_mnl_table <- data.frame(
    Coefficient = c("&beta;<sub>1</sub>", "&beta;<sub>2</sub>",
                    ...
                    "&beta;<sub>7</sub>"),
    Meaning = c("Enrollment Cash", "Monthly Cash", "Override Days",
                "Override Flag", "Minimum Threshold", "Guaranteed Threshold",
                "No Choice"),
...
smc_mnl_kable <- smc_mnl_table %>% 
    kable(format = "html", escape = FALSE,
          col.names = c("Coefficient", "Meaning", "Estimate", "Std Error",
                        "Level", "Unit"),
          align = c("c", "l", "c", "c", "c", "c"),
          caption = "SMC MNL Model Coefficients") %>% 
...
smc_mnl_fig <- data.frame(
    Attribute = factor(
        c("Enrollment Cash", "Monthly Cash", "Override Days", "Override Flag",
          "Min Threshold", "Guaranteed Threshold", "No Choice"),
        levels = c("No Choice", "Guaranteed Threshold", "Min Threshold", 
                   "Override Flag", "Override Days", "Monthly Cash",
                   "Enrollment Cash")
    ),
    Estimate = coef(smc_mnl_model),
    StdError = se(smc_mnl_model)
...
smc_mnl_plot <- smc_mnl_fig %>% 
    ...
    labs(title = "SMC MNL Coefficient Estimates",
         x = "Attribute",
         y = "Estimate") +
    coord_flip() +
...
v2g_mnl_model <- logitr(
    ...
    outcome = "choice",
    obsID   = "obs_id",
    pars    = c("enrollment_cash", "occurrence_cash", "monthly_occurrence",
                "lower_bound", "guaranteed_threshold", "no_choice")
)
...
v2g_mnl_table <- data.frame(
    Coefficient = c("&beta;<sub>1</sub>", "&beta;<sub>2</sub>",
                    "&beta;<sub>3</sub>", "&beta;<sub>4</sub>",
                    "&beta;<sub>5</sub>", "&beta;<sub>6</sub>"),
    Meaning = c("Enrollment Cash", "Occurrence Cash", "Monthly Occurrence",
                "Lower Bound", "Guaranteed Threshold", "No Choice"),
...
v2g_mnl_kable <- v2g_mnl_table %>% 
    kable(format = "html", escape = FALSE,
          col.names = c("Coefficient", "Meaning", "Estimate", "Std Error",
                        "Level", "Unit"),
          align = c("c", "l", "c", "c", "c", "c"),
          caption = "V2G MNL Model Coefficients") %>% 
    kable_styling(bootstrap_options = c("striped", "hover"))
...
v2g_mnl_fig <- data.frame(
    Attribute = factor(
        c("Enrollment Cash", "Occurrence Cash", "Monthly Occurrence",
          "Lower Bound", "Guaranteed Threshold", "No Choice"),
        levels = c("No Choice", "Guaranteed Threshold", "Lower Bound",
                   "Monthly Occurrence", "Occurrence Cash", "Enrollment Cash")
    ),
    Estimate = coef(v2g_mnl_model),
    StdError = se(v2g_mnl_model)
)
...
v2g_mnl_plot <- v2g_mnl_fig %>% 
    ...
    geom_point() +
    geom_errorbar(aes(ymin = LowerBound, ymax = UpperBound), width = 0.2) +
    labs(title = "V2G MNL Coefficient Estimates",
         x = "Attribute",
         y = "Estimate") +
    coord_flip() +
    theme_bw(base_family = "Ubuntu")
